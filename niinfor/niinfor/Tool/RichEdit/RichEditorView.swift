...
@objc public protocol RichEditorDelegate: class {
    ...
    @objc optional func richEditorStateChange(_ editor: RichEditorView, currentStates states: [String])
}

/// RichEditorView is a UIView that displays richly styled text, and allows it to be edited in a WYSIWYG fashion.
@objcMembers open class RichEditorView: UIView, UIScrollViewDelegate, WKNavigationDelegate, WKScriptMessageHandler, UIGestureRecognizerDelegate {
...
//    }
    ...
    open private(set) var userContentController: WKUserContentController
    ...
    private func setup() {
        ...
        userContentController.add(self, name: "html")
        userContentController.add(self, name: "ready")
        userContentController.add(self, name: "editorBar")
        userContentController.add(self, name: "text")
        userContentController.add(self, name: "toast")
        userContentController.add(self, name: "updateLink")
//        userContentController.removeScriptMessageHandler(forName: "html")
...
//        if let filePath = Bundle(for: RichEditorView.self).path(forResource: "rich_editor", ofType: "html") {
        if let filePath = Bundle.main.path(forResource: "rich_editor", ofType: "html", inDirectory:"") {
...
//            webView.load(request)
    ...
    public var html: String {
        get {
            return contentHTML//runJS("RE.getHtml();")
        }
        set {
...
//    }
    ...
    private var placeholderMyText: String = ""
    /// The placeholder text that should be shown when there is no user input.
    open var placeholder: String {
        get { return placeholderMyText }
        set {
            placeholderMyText = newValue
...
//        return string
    ...
    public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        else if message.name == "html" {
            contentHTML = message.body as! String
//            print(message.body)
        }
        else if message.name == "editorBar" {
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        let statePrefix = "re-state://"
        if url?.absoluteString.hasPrefix(statePrefix) == true {
            let states = url?.absoluteString.replacingOccurrences(of: statePrefix, with: "").lowercased().components(separatedBy: ",")
            delegate?.richEditorStateChange?(self, currentStates: states!)
            runJS("RE.getHtml()")
            decisionHandler(WKNavigationActionPolicy.cancel);
            return
        }
        let scrollPrefix = "re-scroll://"
        if url?.absoluteString.hasPrefix(scrollPrefix) == true {
            decisionHandler(WKNavigationActionPolicy.cancel);
            return
        }
    ...
    public func webView(_ webView: UIWebView, shouldStartLoadWith request: URLRequest, navigationType: UIWebViewNavigationType) -> Bool {
        ...
        let statePrefix = "re-state://"
        if request.url?.absoluteString.hasPrefix(statePrefix) == true {
            let states = request.url?.absoluteString.replacingOccurrences(of: statePrefix, with: "").lowercased().components(separatedBy: ",")
            delegate?.richEditorStateChange?(self, currentStates: states!)
            return false
        }
