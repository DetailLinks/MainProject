...
#define DATE_LABEL_INFO_WIDTH UIScreen.mainScreen.bounds.size.width * 0.55
#define DATE_LABEL_INFO_HEIGHT 58.f
...
static NSString* const CLCalendarSelectedDatePrintFormatDefault = @"yyyy.MM";
...
@property (nonatomic,assign) NSInteger swipNumber;
...
-(void)applyCustomDefaults
{
    ...
    NSLog(@"jgjgjgjjhjhj%@  %f", attributes[CLCalendarBackgroundImageColor],  self.selectedDatePrintFontSize);
...
-(UIView *)dailyInfoSubViewContainer
{
    if(!_dailyInfoSubViewContainer){
        _dailyInfoSubViewContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0/*DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT + DATE_VIEW_HEIGHT + DATE_VIEW_MARGIN * 2*/, self.bounds.size.width, DATE_LABEL_INFO_HEIGHT)];
        ...
        [_dailyInfoSubViewContainer addSubview:self.leftBtn];
        [_dailyInfoSubViewContainer addSubview:self.rightBtn];
...
//        _dailyInfoSubViewContainer.backgroundColor = [UIColor redColor];
    }
...
-(UILabel *)dateInfoLabel
{
    ...
    _dateInfoLabel.font = [UIFont fontWithName:@"PingFangSC-Medium" size:20];
                           //systemFontOfSize: self.selectedDatePrintFontSize];
    _dateInfoLabel.textColor = [UIColor colorWithHex:0x3f9cb3];
...
- (UIButton *)leftBtn{
    ...
    if (!_leftBtn) {
       ...
       // _leftBtn.center = CGPointMake(self.dateInfoLabel.center.x  - self.dateInfoLabel.bounds.size.width / 2 - 33/2.f, self.dateInfoLabel.center.y);
        _leftBtn.frame = CGRectMake(CGRectGetMinX(self.dateInfoLabel.frame) - 2*33, self.dateInfoLabel.center.y - 33/2.f, 33, 33);
        
        [_leftBtn setImage:[UIImage imageNamed:@"monthlymeeting_icon_left"] forState:UIControlStateNormal];
        [_leftBtn addTarget:self action:@selector(btnLeftSelet) forControlEvents:UIControlEventTouchUpInside];
    }
...
//        _rightBtn.center = CGPointMake(, );
        _rightBtn.frame = CGRectMake(CGRectGetMaxX(self.dateInfoLabel.frame) - 33, self.dateInfoLabel.center.y - 33/2.f, 33, 33);
        
        [_rightBtn setImage:[UIImage imageNamed:@"monthlymeeting_icon_right"] forState:UIControlStateNormal];
        [_rightBtn addTarget:self action:@selector(btnRightSelet) forControlEvents:UIControlEventTouchUpInside];
    }
    return _rightBtn;
}
...
-(UIView *)dayTitleSubViewContainer
{
    if(!_dayTitleSubViewContainer){
        _dayTitleSubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, DATE_LABEL_INFO_HEIGHT, self.bounds.size.width, DAY_TITLE_VIEW_HEIGHT)];
        _dayTitleSubViewContainer.backgroundColor = [UIColor clearColor];
        _dayTitleSubViewContainer.userInteractionEnabled = YES;
        
//        _dayTitleSubViewContainer.backgroundColor = [UIColor yellowColor];
    }
...
-(UIView *)dailySubViewContainer
{
    if(!_dailySubViewContainer){
        _dailySubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, DATE_LABEL_INFO_HEIGHT +DAY_TITLE_VIEW_HEIGHT+DATE_VIEW_MARGIN, self.bounds.size.width, DATE_VIEW_HEIGHT)];
        _dailySubViewContainer.backgroundColor = [UIColor clearColor];
        _dailySubViewContainer.userInteractionEnabled = YES;
//        _dailySubViewContainer.backgroundColor = [UIColor greenColor];
    }
...
-(UIImageView *)backgroundImageView
{
    ...
    _backgroundImageView.backgroundColor = [UIColor colorWithHex:0xffffff];
...
#define Mark - 星期的颜色字体
    dayTitleLabel.textColor = [UIColor colorWithHex:0x999999];
    //self.dayTitleTextColor;
    //[UIColor redColor];
    //self.dayTitleTextColor;
    dayTitleLabel.textAlignment = NSTextAlignmentCenter;
    dayTitleLabel.font = [UIFont fontWithName:@"PingFangSC-Medium" size:13];
    //[UIFont systemFontOfSize:DAY_TITLE_FONT_SIZE];
...
-(void)btnRightSelet{
    ...
    NSString *titleString = [self.dateInfoLabel.text substringFromIndex:5];
    
    NSString *yearString = [self.dateInfoLabel.text substringToIndex:4];
    
    NSInteger monthNumber = titleString.integerValue + 1;
    ...
    self.dateInfoLabel.text = [NSString stringWithFormat:@"%04ld.%02ld",yearNumber,monthNumber];
...
-(void)renderSwipeDates: (NSDictionary*)param
{
    ...
    if(blnToday){
        ...
        dtStart = (selectedDate)? [selectedDate getWeekStartDate:self.weekStartConfig.integerValue]:[self.startDate addDays:step*7*_swipNumber];
    }
    _swipNumber = 1;
