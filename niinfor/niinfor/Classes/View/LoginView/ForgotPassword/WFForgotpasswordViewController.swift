...
class WFForgotpasswordViewController: BaseViewController {
    ///手机号
   ...
   ///发送验证吗
    @IBAction func sendCodeBtnClick(_ sender: Any) {
        ...
        NetworkerManager.shared.getUpdatePasswordSign(["phoneno":phoneTF.text ?? ""]) { (isSuccess) in
            
            if isSuccess == true {
                
                let vc = WFForgotController()
...
///设置界面
extension WFForgotpasswordViewController : UITextFieldDelegate{
